...
def hard_triplet_loss(student_desc, teacher_desc, margin=1.0):
    ...
    # Positive distance (Student matching Teacher)
    pos_dist = 1 - F.cosine_similarity(student_desc, teacher_desc, dim=1)

    # Negative distance (Student matching OTHER Teachers in batch)
...
def compute_heatmap_loss(student_heat, teacher_out):
    ...
    # 2. Process Teacher to match Student
    if t_heat.shape[1] == 65:
        # A. Softmax over ALL channels (including dustbin)
...
def compute_total_loss(student_out1, student_out2, teacher_out, H, mask, config):
    """
    Computes Total Loss with configurable weights and modes.
    """
    # Unpack Student
    ...
    # --- 2. Distillation Loss ---
    # Check config for distillation mode (default to cosine)
    distill_mode = config["training"].get("distill_mode", "cosine")

    if distill_mode == "triplet":
        loss_distill = hard_triplet_loss(proj1, teacher_desc, margin=1.0)
    else:
    ...
    # Defaults: Heatmap=1000.0 (Strong supervision), others standard
    w_heat = weights.get("heatmap", 1000.0)
    w_distill = weights.get("distill", 1.0)
    w_siamese = weights.get("siamese", 2.0)
    w_quant = weights.get("quant", 0.1)
