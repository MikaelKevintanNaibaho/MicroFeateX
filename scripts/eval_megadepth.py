...
class MegaDepth1500(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for k, v in data.items():
            if k not in ('dataset_name', 'scene_id', 'pair_id', 'pair_names',
                        'size0_hw', 'size1_hw', 'image0', 'image1'):
...
class MicroFeatEXMatcher:
    ...
    def __init__(self, checkpoint_path, config_path, device='cuda', top_k=2000, thresh=0.01):
        self.device = device
        self.top_k = top_k
        self.thresh = thresh
...
def estimate_pose_opencv(kpts0, kpts1, K0, K1, thresh, conf=0.99999):
    ...
    E, mask = cv2.findEssentialMat(kpts0, kpts1, K0, method=cv2.RANSAC,
...
def run_benchmark(matcher, loader, ransac_thr=2.5):
    ...
    for data in tqdm.tqdm(loader, desc="Evaluating"):
        ...
        mkpts0, mkpts1 = matcher.match(
            tensor2bgr(data['image0']),
            tensor2bgr(data['image1'])
        )
